...
_tls_params = jnp.array(_tls_params_list)


# @partial(jax.vmap, in_axes=(1, None), out_axes=1)
def _tls_laf(wavelen: jax.Array, z: float) -> jax.Array:
    ...
    vals = _tls_params[:, 1:4] * jnp.power(w[..., None], jnp.array([1.2, 3.7, 5.5]))
    ...
    mask = w < 2.2
    vals = vals.at[..., 0].multiply(mask)

    mask = (w >= 2.2) & (w < 5.7)
    vals = vals.at[..., 1].multiply(mask)

    mask = w >= 5.7
    vals = vals.at[..., 2].multiply(mask)
...
# @partial(jax.vmap, in_axes=(1, None), out_axes=1)
def _tls_dla(wavelen: jax.Array, z: float) -> jax.Array:
    ...
    # Evaluate power-law terms at every wavelength
    w = wavelen[:, None] / _tls_params[None, :, 0]
    vals = _tls_params[:, 4:] * jnp.power(w[..., None], jnp.array([2.0, 3.0]))
...
def _tlc_laf(wavelen: jax.Array, z: float) -> jax.Array:
...
def _tlc_dla(wavelen: jax.Array, z: float) -> jax.Array:
    ...
    mask = (w >= 1) & (z < 2) & (w < 1 + z)
    vals = 0.211 * (1 + z) ** 2 - 7.66e-2 * (1 + z) ** 2.3 * w ** (-0.3) - 0.135 * w**2
    tau = jnp.where(mask, vals, tau)
    ...
    vals = (
        4.7e-2 * (1 + z) ** 3 - 1.78e-2 * (1 + z) ** 3.3 * w ** (-0.3) - 2.92e-2 * w**3
    )
    tau = jnp.where(mask, vals, tau)
...
def igm_tau(wavelen: jax.Array, z: jax.Array) -> jax.Array:
    ...
    series = _tls_laf(wavelen, z) + _tls_dla(wavelen, z)
    continuum = _tlc_laf(wavelen, z) + _tlc_dla(wavelen, z)
    ...
    mask = jnp.cumsum((continuum > series)[::-1])[::-1].astype(bool)
    ...
    tau = jnp.where(mask, jax.lax.cummax(total, reverse=True), total)
